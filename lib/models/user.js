...
userSchema.statics.getByPublicIdOrga = function (publicId, cb) {
  userModel.findOne({publicId: publicId})
    ...
    function (err, fUser) {
      if (err) {
        ...
        fUser.organizations.forEach(function (organization, i) {
          organization.populateMembers(function (err) {
            if (err) {
              cb(err);
            } else if (i === fUser.organizations.length - 1) {
              cb(null, fUser);
            }
          });
        });
...
userSchema.statics.getByPublicIdFull = function (publicId, cb) {
  userModel.findOne({publicId: publicId})
    ...
    function (err, fUser) {
      if (err) {
        ...
        fUser.organizations.forEach(function (organization, i) {
          organization.populateRooms(function (err) {
            if (err) {
              cb(err);
            } else if (i === fUser.organizations.length - 1) {
              cb(null, fUser);
...
userSchema.methods.safePrint = function () {
  var orgs = [];
  if (this.populated('organizations')) {
