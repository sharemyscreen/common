      ...
      .exec(function (err, fUser) {
        if (err) {
          ...
          fUser.organizations.forEach(function (org, i) {
            org.populate('owner creator members rooms', function (err) {
              if (err) {
                cb(err);
              } else {
                org.rooms.forEach(function (room, j) {
                  room.populate('creator owner members', function (err) {
                    if (err) {
                      cb(err);
                    } else if (i === fUser.organizations.length - 1 && j === org.rooms.length - 1) {
                      cb(null, fUser);
                    }
                  });
                });
...
userSchema.methods.leaveOrganization = function (organization, cb) {
  this.organizations.pull(organization._id);
  this.save(cb);
};
